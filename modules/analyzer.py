...
# 🔁 옛날 이름 호환용 래퍼 (계산은 전부 build_data1_tables에 위임)
def build_datal_tables(df_raw_all: pd.DataFrame):
    """
    구버전 코드 호환용: 예전 이름(build_datal_tables)으로 호출하더라도
    동일한 data_1 테이블을 반환한다.
    """
    return build_data1_tables(df_raw_all)
