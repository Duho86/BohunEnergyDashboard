...
def preprocess_uv_w(
    ...
    # 열 인덱스 기반: C, U, V, W
    org_col = df_raw.columns[2]   # C열 (기관명)
    U_col = df_raw.columns[20]    # U열 (에너지 사용량)
    V_col = df_raw.columns[21]    # V열 (면적당 온실가스)
    W_col = df_raw.columns[22]    # W열 (연면적당 평균)
...
#    - 에너지 사용량 분석/피드백: df_raw(U/V/W) 기반 신규 구현
# ============================================================
...
with tab_dashboard:
    ...
    # 진행중 기능 설명
    with st.expander("🛠️ 현재 반영된 기능 요약"):
        st.markdown(
            """\
- 상단 에너지 사용량 추이(필터 + 그래프 2개) 레이아웃 유지
- 기준배출량 관련 기능/화면 전면 제거
- 하단 에너지 사용량 분석/피드백은 **df_raw(U/V/W)** 기반으로 신규 구현
- NaN/None은 0으로 대체하지 않고, 계산 불가 상황에서만 '-' 표시
"""
        )

    # ========================================================
    # 2-1) 에너지 사용량 추이 (상단 영역 – df_std 기반)
    # ========================================================
    ...
    summary_rows = [
        ["에너지 사용량(현재 기준)", total_U],
        ["면적당 온실가스 배출량", total_V],
        [
            "3개년 평균 대비 증감률(%)",
            None if U_change_rate is None else round(U_change_rate, 1),
        ],
        ["의료시설 평균 에너지 사용량(W)", med_avg],
        ["복지시설 평균 에너지 사용량(W)", wel_avg],
        ["기타시설 평균 에너지 사용량(W)", oth_avg],
    ]
    df_summary = pd.DataFrame(summary_rows, columns=["항목", "값"])
    st.dataframe(
        df_summary.style.format(na_rep="-", formatter={"값": "{:,.1f}".format}),
        use_container_width=True,
    )

    # ---------- 2) 소속기구별 분석 (df_raw 기반 표 전체) ----------
    ...
    if total_U != 0:
        df_group["공단 사용량 분포비율"] = df_group["U_sum"] / total_U * 100
    else:
        df_group["공단 사용량 분포비율"] = pd.NA

    # 평균 에너지 사용량(W) 대비 사용비율 = 기관 W평균 / 시설군 W평균
    def w_ratio(row):
        if row["시설구분"] == "의료시설":
            base = med_avg
        elif row["시설구분"] == "복지시설":
    ...
    df_group["평균 에너지 사용량 대비 사용비율"] = df_group.apply(w_ratio, axis=1)
    ...
    # V합계 기반 '목표 달성을 위한 감축률 분석' (3개년 평균 수준을 목표로 가정)
    total_V = float(raw_df[V_col].sum(skipna=True))
    past_v_values: List[float] = []
    for y in past_years:
        df_past_raw = load_raw_year_data(y)
        if df_past_raw is not None:
            df_past, p_org, p_U, p_V, p_W, _ = preprocess_uv_w(df_past_raw)
            past_v_values.append(float(df_past[p_V].sum(skipna=True)))

    if past_v_values:
        past_avg_V = sum(past_v_values) / len(past_v_values)
        if total_V > 0 and total_V > past_avg_V:
            # 현재 V에서 3개년 평균 수준까지 감축해야 하는 비율
            reduction_needed_pct = (total_V - past_avg_V) / total_V * 100
        else:
    ...
    c1, c2 = st.columns(2)
    c1.metric("기준 달", f"{기준달}월" if 기준달 is not None else "-")
    if reduction_needed_pct is None:
        c2.metric("목표 달성을 위한 감축률 분석", "-")
    else:
        c2.metric(
            "목표 달성을 위한 감축률 분석",
            f"{reduction_needed_pct:,.1f} %",
        )
    ...
    def need_reason(row) -> str:
        ...
        cond2 = (
            pd.notna(row["평균 에너지 사용량 대비 사용비율"])
            and row["평균 에너지 사용량 대비 사용비율"] > 1
        )
    ...
    # ---------- 3-3. 최종 피드백 문장 (기존 스타일로 복원) ----------
    st.markdown("### 공단 전체 분석·코멘트")

    # 선택연도 공단 배출량
    annual_row_sel = annual_total[annual_total["연도"] == selected_year]
    if not annual_row_sel.empty:
        year_total = float(annual_row_sel["연간 온실가스 배출량"].iloc[0])
    else:
        year_total = None
    ...
    # 문장 구성
    year_str = f"{selected_year}년"
    if year_total is not None:
        line1 = f"{year_str} 온실가스 배출량은 총 {year_total:,.0f} tCO2eq로 집계되었습니다."
    else:
        line1 = f"{year_str} 온실가스 배출량은 현재 집계 중입니다."

    if change_pct is not None:
        direction = "증가" if change_pct > 0 else "감소" if change_pct < 0 else "동일"
        line2 = f"전년 대비 {abs(change_pct):.1f}% {direction}한 수치로, 일부 기관의 사용량 변화가 전체 평균에 영향을 준 것으로 분석됩니다."
    else:
        line2 = "전년 대비 증감률을 산정하기 위한 데이터가 부족하여, 추세 분석은 제한적으로 제공됩니다."

    if trend is not None:
        line3 = f"최근 {len(recent_years)}개년 데이터를 보면, 공단 전체 배출량은 전반적으로 **{trend} 추세**를 보이고 있습니다."
    else:
        line3 = "최근 연도별 추세 분석을 위한 데이터가 충분하지 않아, 장기 추이는 향후 데이터를 추가 확보한 뒤 재분석이 필요합니다."

    line4 = (
        "향후 공단은 에너지 사용량과 온실가스 배출이 높은 기관을 중심으로 감축 계획을 강화하고, "
        "우수 절감 실적을 보이는 기관의 사례를 전 기관에 공유하여 관리 체계를 고도화할 필요가 있습니다."
    )
